  ...
  foreach (const string& hierarchy, subsystems.keys()) {
    ...
    if (assign.isError()) {
      string message =
        ...
        path::join(hierarchy, info->cgroup) + "': " + assign.error();
    ...
    const ContainerID& containerId)
{
  ...
  if (!infos.contains(containerId)) {
    // NOTE: We may return a pending future for a non-existent nested
    // container whose ancestor is known to the cgroups isolator.
