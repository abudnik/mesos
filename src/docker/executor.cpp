                ...
                .after(
                    ...
                    [=](Future<Docker::Container> future) {
                      LOG(WARNING) << "Docker inspect timed out after "
                                   << DOCKER_INSPECT_DELAY <<  " for container "
                                   << "'" << containerName << "'";
...
private:
  ...
  void _killTask(const TaskID& taskId_, const Duration& gracePeriod)
  {
    ...
    if (!terminated) {
      ...
      // we need to kill the hanging docker cli subprocess. Discarding a future
      // triggers a callback in the docker library that kills the subprocess.
      if (stop.isPending()) {
        stop.discard();
      }
      ...
      //
      // NOTE: `docker stop` might hang due to a docker bug. Also,
      // invoking `docker stop` might be unsuccessful, in which case the
      // container most probably does not receive the signal. In any case we
      ...
      stop
        .after(KILL_RETRY_INTERVAL, defer(self(), [=](const Future<Nothing>&) {
          LOG(WARNING) << "Docker stop timed out after " << KILL_RETRY_INTERVAL
